...
class Placeholders:
    ProjectName = "BaseNativeApp"
    NativePackage = "me_android_basenativeapp"
...
def init_project (project_package: str):
    ...
    project_native_package = project_package.replace (".", "_")

    main_src = make_path (Paths.SRC_Main,
                          project_package_dirtree, 
                          Files.MainActivity)
    
    nativelib_src = make_path (Paths.SRC_Native, Files.NativeLib)
    ...
    echo ("Ready for initialize Android native project")
    echo ("-------------------------------------------")
    ...
    if not replace_all_in (Placeholders.NativePackage.lower(), 
                           project_native_package, 
                           nativelib_src):
        echo (Messages.Failed)
        echo (Messages.InitializationFailed)
        abort ()
...
if __name__ == "__main__":
    if not DEBUGGING:
        ...
        argv = (None, "me.arithesage.kotlin.android.testnativeapp")
